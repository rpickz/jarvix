...
type Capture struct {
	// Text is the readable content: the window's title line and whatever else
	// the desktop seam can see, already truncated and redacted.
	Text string
...
// Shape is pinned by tests: the reason first, then the base recap — the
// thread's own record still speaks, behind an admission, never behind an
// invention.
const (
...
// RecapPrompt renders the pinned summary prompt. The template is the output
// contract's other half — sentence cap, present state then next step, no
// lists, no preamble — and the delimiters mark the capture as content, never
// instructions (the ADR 0019 stance on injected screen text).
func RecapPrompt(name, captured string) string {
	return fmt.Sprintf("You brief a user re-entering a piece of work they call %q. "+
		...
		"--- window content ---\n%s\n--- end window content ---\n\n"+
		"Say where the work is up to in at most three short sentences: present state first "+
		...
		"out: just the sentences.", name, captured)
}
...
func (s *Service) enrich(ctx context.Context, th Thread, base string, alive map[string]bool) string {
	...
	if !found {
		if th.Recap == RecapAlways && captureErr != nil {
			...
			s.recapSpoken(th, "capture_failed", 0, captureMS, 0, start)
	...
	text := clampRunes(strings.TrimSpace(seen.Text), maxRecapCaptureRunes)
	if text == "" {
		s.recapSpoken(th, "capture_failed", 0, captureMS, 0, start)
		return recapCaptureFallback + " " + base
	}
	...
	modelStart := s.now()
	reply, err := s.summarise(ctx, RecapPrompt(th.Name, text))
	modelMS := s.now().Sub(modelStart).Milliseconds()
	chars := utf8.RuneCountInString(text)
	if err != nil {
		s.recapSpoken(th, "model_failed", chars, captureMS, modelMS, start)
	...
	s.recapSpoken(th, "spoken", chars, captureMS, modelMS, start)
	return summary
}
...
// not what it said (the transient-content acceptance criterion).
func (s *Service) recapSpoken(th Thread, outcome string, chars int, captureMS, modelMS int64, start time.Time) {
	totalMS := s.now().Sub(start).Milliseconds()
	s.log.Info("session recap", "component", "focus", "thread", th.ID,
		"outcome", outcome, "chars", chars,
		"capture_ms", captureMS, "model_ms", modelMS, "total_ms", totalMS)
	if s.publish == nil {
		return
	}
	s.publish("focus.recap", map[string]any{
		"thread":     th.ID,
		"name":       th.Name,
		"outcome":    outcome,
		"chars":      chars,
