...
func TestActivityRowVocabulary(t *testing.T) {
	cases := []struct {
		...
		// A pre-approved run (#162): the row that keeps a standing grant
		// honest — the rule that let it through, and how long that rule
		// stands, beside the verbatim command.
		{"tool.pre_approved", map[string]any{"tool": "shell.run",
			"command": "docker ps --format x", "rule": `configured allow pattern "docker ps"`,
			"pattern": "docker ps", "scope": "always"},
			ActivityRow{Kind: ActivityKindGate, Label: "Ran without asking: shell.run",
				Detail: `pre-approved · configured allow pattern "docker ps" · docker ps --format x`}},
		{"tool.pre_approved", map[string]any{"tool": "intent.run",
			"command": "docker ps", "rule": `conversation allow pattern "docker ps"`,
			"pattern": "docker ps", "scope": "conversation"},
			ActivityRow{Kind: ActivityKindGate, Label: "Ran without asking: intent.run",
				Detail: `pre-approved for this conversation · conversation allow pattern "docker ps" · docker ps`}},
		// An approval that also added a rule reports both halves.
		{"tool.confirmed", map[string]any{"tool": "shell.run", "command": "docker ps",
			"source": "cli", "remembered": "docker ps", "remember_scope": "always"},
			ActivityRow{Kind: ActivityKindGate, Label: "Approved and remembered: shell.run",
				Detail: "docker ps · won't ask again: docker ps"}},
		{"approvals.changed", map[string]any{"action": "forgotten", "pattern": "docker ps",
			"scope": "always", "source": "cli"},
			ActivityRow{Kind: ActivityKindGate, Label: "Pre-approval revoked",
				Detail: "docker ps · pre-approved"}},
		{"approvals.changed", map[string]any{"action": "added", "pattern": "docker ps",
			"scope": "always", "source": "card"},
			ActivityRow{Kind: ActivityKindGate, Label: "Pre-approval added",
				Detail: "docker ps · pre-approved"}},
