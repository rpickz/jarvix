...
| `status.get` | — | `{state, session_id, version, protocol, ptt, policy, warm, last_timings, last_typing}` | `ptt` is `"daemon"` when jarvixd watches the hold-to-talk chord itself (keybinding toggles must then no-op) or `"external"` when keybindings drive activation. `policy` is the effective tool permission policy: `{default, confirm_timeout_sec, remember_for_conversation, tools: {name: decision}}`. `warm` lists the supervised engine workers (ADR 0018), one entry per engine: `{name, running, pid, rss_mb, uptime_sec, restarts, last_error}` — empty when `performance.warm_engines` is off. `last_timings` is the most recent `session.timings` payload (plus its `session_id`), or `null` before any session has finished; `jarvix status --last` prints it, alongside the desktop context from `context.last`. `last_typing` is the most recent `typing.audit` payload, or `null` when typing is off or nothing has been typed since the daemon started — it never carries the typed text |
