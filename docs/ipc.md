...
| `knowledge.status` | — | `{enabled, path?, feeds?}` | The feed cache (ADR 0030), read-only: one entry per configured `[[knowledge.feeds]]` table — `{name, mode, inject, has_value, stale, failing, value?, fetched?, age_sec?, failing_since?, attempts?, last_error?}` — in declaration order. `value` travels here deliberately (the user's own data over their own 0600 socket, the `memory.list` precedent) and nowhere else: logs and events carry names and counts only. `jarvix doctor` composes its "N fresh, M failing since …" line from this. The feed tables themselves are hand-edited TOML like `[[routines]]` and are not editable over IPC |
...
| `knowledge.injected` | `{feeds, trimmed, est_tokens}` | Cached feed values were offered to the model for this turn (ADR 0030), beside the desktop context. **Counts and estimates only, never values** — feed values may be sensitive; they are fetched with `knowledge.status`, over the socket, on request. Published only on turns that reach the model, and only when at least one opted-in feed had a value |
