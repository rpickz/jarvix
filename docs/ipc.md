...
| `context.captured` | `{sources, duration_ms}` | Desktop context was gathered for this turn (ADR 0018), immediately before the provider request. `sources` is an array of `{source, chars, truncated, redacted}` — **sizes and flags only, never content**, because events fan out to every connected client. Published only on turns that reach the model: a matched deterministic intent gathers nothing. Fetch the captured text itself with `context.last` |
