...
| `knowledge.status` | — | `{enabled, path?, feeds?}` | The feed cache (ADR 0031), read-only: one entry per configured `[[knowledge.feeds]]` table — `{name, mode, inject, has_value, stale, failing, value?, fetched?, age_sec?, failing_since?, attempts?, last_error?}` — in declaration order. `value` travels here deliberately (the user's own data over their own 0600 socket, the `memory.list` precedent) and nowhere else: logs and events carry names and counts only. `jarvix doctor` composes its "N fresh, M failing since …" line from this. The feed tables themselves are hand-edited TOML like `[[routines]]` and are not editable over IPC |
