...
| `routines.list` | — | `{routines}` | The configured routines (ADR 0025), read-only: each entry is `{name, phrases, steps}` where `steps` is a count. The steps themselves are hand-edited TOML (`[[routines]]` in config.toml) and are not exposed or editable over IPC in v1 |
