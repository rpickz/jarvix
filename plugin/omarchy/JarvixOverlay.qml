...
import qs.Commons
import qs.Ui
// The wording of an outcome is decided in Go and rendered here (ADR 0013):
// the overlay and the conversation window must not word the same nothing two
// ways. Generated by `go generate ./internal/desktop`.
...
Item {
  ...
  // noticeMessage is an honest nothing, not a fault: the capture produced no
  // words (issue #191). It gets its own property rather than riding
  // errorMessage precisely so it cannot inherit the urgent colour, the longer
  // linger and the "Jarvix hit a problem" headline that go with it.
  ...
  function handleEvent(method, params) {
    ...
    case "state.changed":
      ...
      sessionState = next
      if (next === "idle" && (response !== "" || errorMessage !== "" || noticeMessage !== "")) {
        lingering = true
        // The notice lingers for the ordinary 1800 ms, not the error's 4000:
        // "I didn't catch that" is a sentence you read once and act on by
        // pressing the key again, and holding it longer would make a quiet
        // nothing feel like an incident.
    ...
    case "session.nothing_heard":
      // The capture produced nothing. Said out loud here, in the place the
      // user is already looking, because the alternative — the overlay simply
      // closing — is indistinguishable from the press not having registered.
      // Never errorMessage: nothing failed.
      noticeMessage = BarState.pendingTurnNothingHeard
      break
  ...
  readonly property string statusLabel: {
    ...
    if (noticeMessage !== "") return "Nothing heard"
    switch (sessionState) {
    ...
    BorderSurface {
      ...
      Column {
        ...
        Text {
          ...
          color: Util.alpha(Color.popups.text, root.response !== "" || root.errorMessage !== "" || root.noticeMessage !== "" ? 1.0 : 0.7)
        }
